...
Flusso:
  ...
  2. Estrae i way con tag highway=path + sac_scale=* (sentieri CAI)
...
def _sanitize_str(value: str | None, max_len: int) -> str | None:
    ...
    if value is None:
        return None
    return value.strip()[:max_len] if value.strip() else None
...
class TrailHandler(osmium.SimpleHandler):
    ...
    def way(self, w):
        ...
        if "highway" not in tags or tags.get("highway") not in ("path", "footway", "track"):
            return
        ...
        try:
            ...
            return  # geometria non valida, salta

        try:
            line = osmium.geom.shape(wkb) if hasattr(osmium.geom, "shape") else None
            # Fallback: costruiamo la geometria dai nodi
        ...
        bbox = line.bounds  # (min_x, min_y, max_x, max_y)
        ...
        self.trails.append({
            ...
            "bbox":      list(bbox),
            "updated_at": str(w.version),         # timestamp non disponibile qui, usa version
        })
    ...
    def node(self, n):
        ...
        if tourism == "alpine_hut":
            ...
            self.shelters.append({
                ...
                "elevation_m":   int(ele) if ele and ele.isdigit() else None,
...
def sample_elevation(coords: list[tuple], dem_paths: list[Path]) -> dict[str, Any]:
    ...
    Restituisce: distance_m, gain, loss, min, max.

    Se nessun tile DEM copre il sentiero, restituisce None per tutti i campi.
    """
    try:
        import rasterio
        ...
        return {"distance_m": None, "elevation_gain_m": None,
                "elevation_loss_m": None, "elevation_min_m": None, "elevation_max_m": None}

    elevations = []
    for lon, lat in coords:
        ele = _sample_single_point(lon, lat, dem_paths)
        if ele is not None:
            elevations.append(ele)

    if not elevations:
        return {"distance_m": _calc_distance(coords),
...
def _calc_distance(coords: list[tuple]) -> int:
    """Calcola la lunghezza del tracciato in metri (proiezione WGS84 -> metri)."""
    if len(coords) < 2:
        return 0
...
def extract_all(osm_paths: list[Path], dem_paths: list[Path]) -> dict:
    ...
    # Deduplicazione per osm_id (il sentiero può apparire in più file regionali)
    all_trails   = {t["osm_id"]: t for t in all_trails}.values()
    all_shelters = {s["osm_id"]: s for s in all_shelters}.values()
    all_water    = {w["osm_id"]: w for w in all_water}.values()
    all_parkings = {p["osm_id"]: p for p in all_parkings}.values()

    return {
        "trails":        list(all_trails),
        "shelters":      list(all_shelters),
        "water_sources": list(all_water),
        "parkings":      list(all_parkings),
    }
